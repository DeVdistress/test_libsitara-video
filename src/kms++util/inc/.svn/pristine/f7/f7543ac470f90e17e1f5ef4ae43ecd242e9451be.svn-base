...
public:
	...
	VideoStreamer* get_capture_streamer(bool for_vpe = false);
	VideoStreamer* get_output_streamer(bool for_vpe = false);
...
public:
	...
	void set_format(kms::PixelFormat fmt, uint32_t width, uint32_t height, uint8_t use_deinterlace = 0);
	...
	void queue(kms::Framebuffer* fb, const uint32_t *field = nullptr, const uint32_t *buf_type = nullptr);
	kms::Framebuffer* dequeue(uint32_t *field = nullptr, uint32_t *idx_ = nullptr);
	void stream_on();
	void stream_off();

	const std::vector<kms::Framebuffer*>& getVideoBuffer() const { return m_fbs;}
	void nullVideoBuffer(uint32_t idx) { if(idx < m_fbs.size()) m_fbs[idx] = nullptr;}
